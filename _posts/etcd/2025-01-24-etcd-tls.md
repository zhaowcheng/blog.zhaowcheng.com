---
title: ETCD 配置 TLS
date: 2025-01-24 15:28:00 +0800
categories: [ETCD]
tags: [etcd, network, ssl, tls]
---

前提知识： [SSL/TLS 笔记](https://blog.zhaowcheng.com/posts/ssl-notes/)
...
+------------------+---------+-------+---------------------------+---------------------------+------------+
| e36d8869dc221ffe | started | node1 | http://192.168.70.41:2380 | http://192.168.70.41:2379 |      false |
| 243fcfa74ec0736a | started | node2 | http://192.168.70.15:2380 | http://192.168.70.15:2379 |      false |
| c8ad351a3ef67e9e | started | node3 | http://192.168.70.14:2380 | http://192.168.70.14:2379 |      false |
+------------------+---------+-------+---------------------------+---------------------------+------------+
```
...
生成 `节点证书和私钥`：
```console
$ python gen_etcd_certs.py -c gen_server_cert -s ./certs -k certs/root.key -t certs/root.cert -i 192.168.70.41
Saved: ./certs/192.168.70.41.key
Saved: ./certs/192.168.70.41.cert

$ python gen_etcd_certs.py -c gen_server_cert -s ./certs -k certs/root.key -t certs/root.cert -i 192.168.70.15
Saved: ./certs/192.168.70.15.key
Saved: ./certs/192.168.70.15.cert

$ python gen_etcd_certs.py -c gen_server_cert -s ./certs -k certs/root.key -t certs/root.cert -i 192.168.70.14
Saved: ./certs/192.168.70.14.key
Saved: ./certs/192.168.70.14.cert
```
...
生成 `节点证书和私钥`：
```console
$ openssl genrsa -out 192.168.70.41.key 2048
Generating RSA private key, 2048 bit long modulus
.......................+++
..................................+++
e is 65537 (0x10001)
$ openssl req -new -sha256 -key 192.168.70.41.key -out 192.168.70.41.csr -subj "/C=CN/ST=CQ/L=YB/O=BC/OU=ZWC"
$ echo "subjectAltName = @names\n[names]\nIP.1 = 127.0.0.1\nIP.2 = 192.168.70.41" > 192.168.70.41.ext
$ openssl x509 -req -days 3650 -sha256 -CA  root.cert -CAkey root.key  -CAserial root.srl  -CAcreateserial -in 192.168.70.41.csr -out 192.168.70.41.cert -extfile 192.168.70.41.ext
Signature ok
subject=/C=CN/ST=CQ/L=YB/O=BC/OU=ZWC
Getting CA Private Key

$ openssl genrsa -out 192.168.70.15.key 2048
Generating RSA private key, 2048 bit long modulus
.......................+++
..................................+++
e is 65537 (0x10001)
$ openssl req -new -sha256 -key 192.168.70.15.key -out 192.168.70.15.csr -subj "/C=CN/ST=CQ/L=YB/O=BC/OU=ZWC"
$ echo "subjectAltName = @names\n[names]\nIP.1 = 127.0.0.1\nIP.2 = 192.168.70.15" > 192.168.70.15.ext
$ openssl x509 -req -days 3650 -sha256 -CA  root.cert -CAkey root.key  -CAserial root.srl  -CAcreateserial -in 192.168.70.15.csr -out 192.168.70.15.cert -extfile 192.168.70.15.ext
Signature ok
subject=/C=CN/ST=CQ/L=YB/O=BC/OU=ZWC
Getting CA Private Key

$ openssl genrsa -out 192.168.70.14.key 2048
Generating RSA private key, 2048 bit long modulus
.......................+++
..................................+++
e is 65537 (0x10001)
$ openssl req -new -sha256 -key 192.168.70.14.key -out 192.168.70.14.csr -subj "/C=CN/ST=CQ/L=YB/O=BC/OU=ZWC"
$ echo "subjectAltName = @names\n[names]\nIP.1 = 127.0.0.1\nIP.2 = 192.168.70.14" > 192.168.70.14.ext
$ openssl x509 -req -days 3650 -sha256 -CA  root.cert -CAkey root.key  -CAserial root.srl  -CAcreateserial -in 192.168.70.14.csr -out 192.168.70.14.cert -extfile 192.168.70.14.ext
Signature ok
subject=/C=CN/ST=CQ/L=YB/O=BC/OU=ZWC
Getting CA Private Key
```
...
| 节点 | 需要上传的文件 |
| --- | ----------- |
| 192.168.70.41 | `root.cert`, `root.key`, `client.cert`, `client.key`, `192.168.70.41.cert`, `192.168.70.41.key` |
| 192.168.70.15 | `root.cert`, `root.key`, `client.cert`, `client.key`, `192.168.70.15.cert`, `192.168.70.15.key` |
| 192.168.70.14 | `root.cert`, `root.key`, `client.cert`, `client.key`, `192.168.70.14.cert`, `192.168.70.14.key` |

## 更新配置

节点 `192.168.70.41` 配置文件需要更新的配置：
```YAML
listen-peer-urls: https://0.0.0.0:2380
listen-client-urls: https://0.0.0.0:2379
advertise-client-urls: https://192.168.70.41:2379
client-transport-security:
  # 服务端证书，在 TLS 握手过程中提供给客户端。
  cert-file: /path/to/192.168.70.41.cert
  # 服务端私钥，用来解密客户端使用服务端公钥加密发送的数据。
  key-file: /path/to/192.168.70.41.key
  # 是否要求客户端访问时提供客户端证书。
...
节点 `192.168.70.15` 配置文件需要更新的配置：
```YAML
listen-peer-urls: https://0.0.0.0:2380
listen-client-urls: https://0.0.0.0:2379
advertise-client-urls: https://192.168.70.15:2379
client-transport-security:
  # 服务端证书，在 TLS 握手过程中提供给客户端。
  cert-file: /path/to/192.168.70.15.cert
  # 服务端私钥，用来解密客户端使用服务端公钥加密发送的数据。
  key-file: /path/to/192.168.70.15.key
  # 是否要求客户端访问时提供客户端证书。
  client-cert-auth: true
  # 受信任的 CA 证书（root 证书/根证书），用来验证客户端证书。
  trusted-ca-file: /path/to/root.cert
...
节点 `192.168.70.14` 配置文件需要更新的配置：
```YAML
listen-peer-urls: https://0.0.0.0:2380
listen-client-urls: https://0.0.0.0:2379
advertise-client-urls: https://192.168.70.14:2379
client-transport-security:
  # 服务端证书，在 TLS 握手过程中提供给客户端。
  cert-file: /path/to/192.168.70.14.cert
  # 服务端私钥，用来解密客户端使用服务端公钥加密发送的数据。
  key-file: /path/to/192.168.70.14.key
  # 是否要求客户端访问时提供客户端证书。
  client-cert-auth: true
  # 受信任的 CA 证书（root 证书/根证书），用来验证客户端证书。
  trusted-ca-file: /path/to/root.cert
...
```console
$ etcdctl member update e36d8869dc221ffe --peer-urls="https://192.168.70.41:2380"
Member e36d8869dc221ffe updated in cluster 77feb499f2ffa1c8

$ etcdctl member update 243fcfa74ec0736a --peer-urls="https://192.168.70.15:2380"
Member 243fcfa74ec0736a updated in cluster 77feb499f2ffa1c8

$ etcdctl member update c8ad351a3ef67e9e --peer-urls="https://192.168.70.14:2380"
...
|        ID        | STATUS  | NAME  |        PEER ADDRS          |       CLIENT ADDRS         | IS LEARNER |
+------------------+---------+-------+----------------------------+----------------------------+------------+
| e36d8869dc221ffe | started | node1 | https://192.168.70.41:2380 | https://192.168.70.41:2379 |      false |
| 243fcfa74ec0736a | started | node2 | https://192.168.70.15:2380 | https://192.168.70.15:2379 |      false |
| c8ad351a3ef67e9e | started | node3 | https://192.168.70.14:2380 | https://192.168.70.14:2379 |      false |
+------------------+---------+-------+----------------------------+----------------------------+------------+
```
